...
//    чомусь так не працює, залишає mockMessageDto = null. Треба так, як нижче
*/
...
*/
/*
        1. Здесь Spring Boot c помощью @WebMvcTest(PersonController.class) создаёт
        фейковое окружение с настроенным Spring MVC и входящим в него Jackson,
        причём именно в том виде, в каком они настроены в реальном приложении.
        2. Далее мы создаём запрос с помощью mockMvc, кстати, его можно настраивать.
        3. А далее мы пишем набор matсher-ов, которые проверяют запрос, здесь можно
        проверить также и контент, и HTTP-заголовки.
        - Вот такими манипуляциями можно проверить и View-слой SpringBoot приложения.
        А как проверить DAO/Repository-слой, мы узнаем в следующей заметке.
*/
    }
